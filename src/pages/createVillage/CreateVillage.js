...
const CreateVillage = () => {
  ...
  const fatchMandals = async () => {
    try {
      const response = await getApiCall('village/getAllMandals')
  ...
  const fetchDistrict = async (value) => {
    ...
    try {
      const response = await getApiCallWithParams(`/village/getDistrictById/${value.district_id}`)
  ...
  const fetchState = async (value) => {
    ...
    try {
      const { state } = await getApiCallWithParams(`/village/getStateById/${value.state_id}`)
  ...
  const onSubmit = async (data) => {
    ...
    const response = postApiCall('/village/createVillage', villageData)
    response.then((res) => {
      console.log('res', res)
      if (res.message) {
        ...
        <Box sx={{ my: 2 }}>
          <TextField
            ...
            {...register("villageName", {
              required: "Village name is required", validate: async (value) => {
                try {
                  const res = await getApiCallWithParams(`village/checkVillage/${value}`);
